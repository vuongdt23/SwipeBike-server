...
TripRequestRouter.post ('/acceptRequest/:requestId', (req, res, next) => {
  ...
  prisma.tripRequest
    ...
    .then (TripRequest => {
      if (
        TripRequest.RequestTargetId !== Number.parseInt (userProfile.UserId)
      ) {
      ...
      prisma.trip.create ({data: TripToCreate}).then (createTripResult => {
        ...
        const candidateIds = [
          TripRequest.CandidateTripSent,
          TripRequest.CandidateTripFrom,
        ];

        prisma.candidateTrip.updateMany ({
          where: {CandidateTripId: {in: candidateIds}},
          data: {TripStatusId: 9},
        });
